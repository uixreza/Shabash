      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-primary/10 bg-background p-8 shadow-2xl shadow-primary/20">
        ...
        {step === 1 ? (
          <div className="flex flex-col items-center gap-6">
            ...
            <h2 className="text-xl font-bold text-foreground">حالت نمایش را انتخاب کنید</h2>
            <p className="-mt-3 text-sm text-foreground/50">رابط کاربری مورد نظر خود را انتخاب کنید</p>

            <div className="flex gap-4">
              <button
                onClick={() => setTheme("light")}
                className={`flex w-32 flex-col items-center gap-3 rounded-2xl border-2 p-5 transition-all ${
                  theme === "light" ? "border-primary bg-primary/5 shadow-sm shadow-primary/20" : "border-primary/10 hover:border-primary/30"
                }`}
              >
                ...
                <span className="text-sm font-medium text-foreground">روشن</span>
              </button>

              <button
                onClick={() => setTheme("dark")}
                className={`flex w-32 flex-col items-center gap-3 rounded-2xl border-2 p-5 transition-all ${
                  theme === "dark" ? "border-primary bg-primary/5 shadow-sm shadow-primary/20" : "border-primary/10 hover:border-primary/30"
                }`}
              >
                ...
                <span className="text-sm font-medium text-foreground">تاریک</span>
              </button>
            </div>

            <button onClick={() => setStep(2)} className="w-full rounded-xl bg-primary py-3 text-sm font-medium text-white transition-all hover:bg-primary/90 active:scale-[0.98]">
              ادامه
            </button>
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="-mt-1 text-sm text-foreground/50">لطفا اطلاعات خود را وارد کنید</p>
            </div>
